...
import { X, Loader2, Globe, Type } from "lucide-react";
import { createBot } from "@/lib/adminApi";
import { DEFAULTS } from "@/lib/defaults";

export function AdvancedCreateModal({
  onClose,
  onSaved,
}: {
  onClose: () => void;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-border bg-surface p-6 shadow-2xl animate-in zoom-in-95">
        <button
          onClick={onClose}
          disabled={isSaving}
          className="absolute right-4 top-4 rounded-full p-1.5 text-muted hover:bg-panel hover:text-fg transition-colors disabled:opacity-50"
        >
          <X className="h-5 w-5" />
        </button>
        
        <h2 className="mb-1 text-xl font-bold text-fg">Create Agent</h2>
        <p className="mb-6 text-[14px] text-muted">
          Provide the basics. You can configure knowledge and appearance later.
        </p>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <Globe className="absolute left-3 top-2.5 h-4 w-4 text-faint" />
              <input
                type="url"
                disabled={isSaving}
                value={websiteUrl}
            ...
            <p className="mt-1.5 text-[12px] text-muted">
              We'll use this later to automatically scan your website.
            </p>
          </div>
        ...
        <div className="mt-8 flex items-center justify-end gap-3">
          <button
            ...
            onClick={onClose}
            disabled={isSaving}
            className="rounded-xl px-4 py-2 text-[14px] font-[650] text-muted hover:text-fg disabled:opacity-50 transition-colors"
          >
            Cancel
          </button>
          <button
