...
export function SiteHeader({ cta = true }: { cta?: boolean }) {
  ...
  // Move focus into the drawer on open, back to the toggle on close. Skips
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-glass backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-[1240px] items-center justify-between px-6 py-3.5 sm:px-9">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <Link
              href="/sign-in"
              className="hidden rounded-r1 px-3 py-2 text-[14px] font-[600] text-muted transition-colors hover:text-fg lg:block whitespace-nowrap"
            >
              Sign in
            </Link>
            {cta && (
              <Link
                href="/sign-up"
                className="hidden sm:inline-flex items-center rounded-r1 bg-gradient-to-br from-accent to-accent-strong px-4 py-2 text-[14px] font-[650] text-white shadow-[0_6px_16px_-6px_var(--accent)] transition-transform hover:-translate-y-0.5 whitespace-nowrap"
              >
                Build your bot
              </Link>
            )}
            ...
            <button
              ref={menuBtnRef}
              type="button"
              className="tap grid h-9 w-9 place-items-center rounded-xl border border-border bg-surface/80 text-fg hover:border-accent transition-colors lg:hidden"
              onClick={() => setOpen((v) => !v)}
