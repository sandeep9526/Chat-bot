...
// Extremely basic syntax highlighting for HTML/JS
const highlightSnippet = (code: string) => {
  return code
    .split("\n")
    .map((line, i) => {
      // Very crude regex for strings, HTML tags, and comments
      let highlighted = line
        .replace(/("[^"]*")/g, '<span class="text-emerald-400">$1</span>') // strings
        .replace(/(<\/?script.*?>)/g, '<span class="text-blue-400">$1</span>') // tags
        .replace(/(data-[\w-]+)=/g, '<span class="text-purple-400">$1</span>=') // attrs
