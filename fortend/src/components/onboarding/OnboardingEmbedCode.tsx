...
/**
 * Same look as Studio's EmbedCode.tsx, but parameterized by the bot the
 * wizard just created — Studio's version always embeds the fixed demo
 * BOT_ID, so it can't be reused as-is for a specific tenant.
 */
export function OnboardingEmbedCode({ botId, config }: OnboardingEmbedCodeProps) {
  const [copied, setCopied] = useState(false);

  const embedText = useMemo(() => buildEmbedText(config, botId), [config, botId]);
  const embedHtml = useMemo(() => buildEmbedHtml(config, botId), [config, botId]);
  ...
  return (
    <div className="overflow-hidden rounded-[16px] border border-code-border bg-code-bg">
